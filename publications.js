const publicationFiles = [
  ...
  },
  {
    ...
    "file": "papers/2024-17.png",
    "preview": "preview-2024-17.jpg"
  },
  {
    "year": "2024",
    "type": "pdf",
    "title": "Seasonal WaveNetLSTM",
    "file": "papers/2024-18.pdf",
  ...
  },
  {
    ...
    "file": "papers/2026-39.png",
    "preview": "preview-2026-39.jpg"
  },
  {
    "year": "2026",
    "type": "pdf",
    "title": "Circular Biorefinery Pathways for Pesticide Wastewater",
    "file": "papers/2026-40.pdf",
...
function previewFallbackMarkup(item) {
  return `<div class="paper-preview generated-preview"><span>${escapeHtml(item.year)}</span><strong>${escapeHtml(item.type.toUpperCase())}</strong><p>${escapeHtml(item.title)}</p></div>`;
}
...
function renderDownloads(filter = activePublicationFilter, query = activePublicationSearch) {
  ...
  downloadGrid.innerHTML = visibleFiles.map((item) => {
    ...
    const preview = item.preview
      ? `<img loading="lazy" src="${previewUrl(item.preview)}" alt="First page preview of ${safeTitle}" onerror="this.replaceWith(this.nextElementSibling)">${previewFallbackMarkup(item)}`
      : previewFallbackMarkup(item);
